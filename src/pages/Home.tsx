...
export default function Home() {
  ...
  // 📜 Saved Analyses State (Local-first + Firestore Cloud Sync)
  const [savedAnalyses, setSavedAnalyses] = useState<SavedAnalysis[]>([]);
  const [isLoadingSaved, setIsLoadingSaved] = useState(false);

  const getStorageKey = () => (user ? `TimeComplexityAI_history_${user.uid}` : 'TimeComplexityAI_history_guest');
  ...
  const fetchSavedAnalyses = async () => {
    ...
    try {
      ...
      const colRef = firestore.collection(db, 'users', user.uid, 'analyses');
      ...
      console.warn("Firestore sync fetch notice:", error);
    } finally {
  ...
  const saveAnalysisRecord = async (codeToSave: string, analysisRes: AnalysisResult) => {
    const newRecord: SavedAnalysis = {
      id: 'local_' + Date.now(),
      code: codeToSave,
      complexity: analysisRes.complexity || 'Unknown',
      complexityClass: analysisRes.complexityClass || 'O(1)',
      spaceComplexity: analysisRes.spaceComplexity || 'O(1)',
      explanationPoints: analysisRes.explanationPoints || [],
      createdAt: new Date(),
    };

    // 1. Optimistic Local Save
    setSavedAnalyses((prev) => {
      const filtered = prev.filter((p) => p.code.trim() !== codeToSave.trim());
    ...
    if (user) {
      try {
        ...
        const docRef = await firestore.addDoc(firestore.collection(db, 'users', user.uid, 'analyses'), {
          code: codeToSave,
          complexity: analysisRes.complexity || 'Unknown',
          complexityClass: analysisRes.complexityClass || 'O(1)',
          spaceComplexity: analysisRes.spaceComplexity || 'O(1)',
          explanationPoints:
            analysisRes.explanationPoints && analysisRes.explanationPoints.length > 0
              ? analysisRes.explanationPoints
              : ['Analysis generated by TimeComplexityAI'],
          createdAt: firestore.serverTimestamp(),
        });

        setSavedAnalyses((prev) =>
          prev.map((item) => (item.id === newRecord.id ? { ...item, id: docRef.id } : item))
        );
      } catch (err) {
  ...
  const handleSaveAnalysis = async () => {
    ...
    setIsSaving(true);
    try {
      await saveAnalysisRecord(analyzedCode || code, result);
      setIsSaved(true);
    } catch (error) {
